        ...
        if len(args.property) == 0:
            ...
            if args.property[0] == 'address':
                ...
                if len(args.property) == 1:                 # python customer.py ID address
                    sql = "SELECT address FROM address WHERE cid=%(cid)s ORDER BY cid;"
                    ...
                    if args.property[1] == '-c' or args.property[1] == '--create':
                        ...
                        if len(args.property) == 3:
                            sql = "DELETE FROM address WHERE cid = \
                            (SELECT cid FROM address WHERE cid=%(cid)s ORDER BY cid LIMIT 1 OFFSET %(n)s);"
                            cur.execute(sql, {"cid":args.id, "n": int(args.property[2]) - 1})
                            conn.commit()
                            print('Removal complete') #위에 부분 id 를 다 cid로 바꿔야 함. 
                        else:
